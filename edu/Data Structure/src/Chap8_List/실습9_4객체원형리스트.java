...
class CircularList {
	...
	public void Show() { // 전체 리스트를 순서대로 출력한다.
		...
		while (curNode.link !=first){
			System.out.println((num++) + " : " +curNode.data + " "); //출력!
			curNode = curNode.link; //다음걸로 이동
		}
		System.out.println((num++) + " : " +curNode.data + " ");
	}
	...
	public void Add(SimpleObject3 element, Comparator<SimpleObject3> cc){ // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
		...
		if (first.data.isNull()) {
			...
			if (cc.compare(preNode.data, element)<=0) {
	...
	public boolean Search(SimpleObject3 element, Comparator<SimpleObject3> cc) { // 전체 리스트를 순서대로 출력한다.
		Node3 curNode = first.link;
		do{
			if(cc.compare(element, curNode.data)==0) return true;
			else curNode = curNode.link; //다음걸로 이동
		}while(curNode != first.link) ;
		return false;
	}
	void Merge(CircularList b, Comparator<SimpleObject3> cc) {
        //구현 필요
		System.out.println("머지문시작");
		Node3 curA = this.first;
		Node3 curB = b.first;
		Node3 lastA = this.first;
		while(lastA.link != this.first) lastA = lastA.link;
		Node3 lastB = b.first;
		while(lastB.link != b.first) lastB = lastB.link;
		lastA.link = null; //break circular links to make linear lists
		lastB.link = null;
		Node3 dummy = new Node3(new SimpleObject3());
		Node3 res = dummy;
		while(curA !=null && curB !=null) {
			if (cc.compare(curA.data, curB.data)<=0) {
				res.link = curA;
				curA =curA.link;
			}else{
				res.link = curB;
		...
		this.first = dummy.link; //새로운 헤드 설정
		Node3 mergeTail = this.first;
		while(mergeTail.link!=null) {
			mergeTail = mergeTail.link;
		}
		mergeTail.link = this.first;
    }
}
