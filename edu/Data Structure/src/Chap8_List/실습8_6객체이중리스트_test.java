...
class DoubledLinkedList2 {
	...
	public boolean search(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		Node4 temp = first.rlink;
		while(temp.rlink != first) {
			if(c.compare(obj, temp.data)==0) return true;
			temp = temp.rlink;
		}
		return false;
	}
	...
	// --- 전체 노드 표시 ---//
	public void show() {
//		System.out.println(first.data); 더미니깐 출력 안해도 됨.
		Node4 temp = first.rlink;
		int num = 0;
		while(temp.rlink != first && num++ <=30) {
			System.out.println(temp.data);
			temp = temp.rlink;
		}
		if(num >30) {
			System.out.println("출력초과");
		}
	}
	...
	public void add(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		...
		Node4 temp = first.rlink;
		if(this.isEmpty() || c.compare(temp.data, obj)>=0) { //처음 삽입
			first.rlink = newNode;
			newNode.rlink = temp;
			newNode.llink = first;
			temp.llink = newNode;
			...
			while (c.compare(temp.data, obj) <= 0 && temp.rlink != first) {
				temp = temp.rlink;
			}
			Node4 preNode = temp.llink;
	...
	public void delete(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		Node4 temp = first.rlink;
			while(temp.rlink != first) {
				Node4 preNode = temp.llink;
				Node4 nextNode = temp.rlink;
				if(c.compare(obj, temp.data)==0) {
					System.out.println("삭제완료 삭제된 데이터는 " + temp.data);
	...
	public DoubledLinkedList2 merge_NewList(DoubledLinkedList2 lst2, Comparator<SimpleObject2> cc) {
		...
		Node4 tempA = this.first.rlink, tempB = lst2.first.rlink, tempC = lst3.first;
		while(tempA.rlink != this.first && tempB.rlink != lst2.first) {
			if(cc.compare(tempA.data, tempB.data)<=0) {
				tempC.rlink = tempA;
				tempA.llink = tempC;
				tempA = tempA.rlink;
			}else {
		...
		if(tempA.rlink == this.first) { // tempA가 텅비면
			tempC.rlink = tempB;
			tempB.llink = tempC;
			while(tempB.rlink != lst2.first) {
				tempB = tempB.rlink;
			}
			tempB.rlink = lst3.first; //tempB의 마지막을 새로운 리스트의 first와 연결
			lst3.first.llink = tempB;
		}else if(tempB.rlink == lst2.first) { // tempB가 텅비면
			tempC.rlink = tempA;
			tempA.llink = tempC;
			while(tempA.rlink != this.first) {
				tempA = tempA.rlink;
			}
			tempA.rlink = lst3.first;
			lst3.first.llink = tempA;
	...
	void merge_InPlace(DoubledLinkedList2 b, Comparator<SimpleObject2> cc) {
		...
		Node4 dum = temp;
		while(p.rlink != this.first && q.rlink != b.first) {
		...
		if(p.rlink == this.first) { // tempA가 텅비면
			temp.rlink = q;
			q.llink = temp;
			while(q.rlink != b.first) {
				q = q.rlink;
			}
			q.rlink = this.first; //tempB의 마지막을 새로운 리스트의 first와 연결
			this.first.llink = q;
		}else if(q.rlink == b.first) { // tempB가 텅비면
			temp.rlink = p;
			p.llink = temp;
		}
		first.rlink = dum.rlink;
		dum.rlink.llink = first;
	}
}
...
//				}
				...
				lst4.show();
				lst4.merge_InPlace(lst1, SimpleObject2.NO_ORDER);
				//merge 실행후 show로 결과 확인 - 새로운 노드를 만들지 않고 합병 - 난이도 상
	...
	static void makeSimpleObjects3(SimpleObject2 []simpleobjects) {
        simpleobjects[0] = new SimpleObject2("s5", "kkk", "240608");
        simpleobjects[1] = new SimpleObject2("s2", "aabboe3", "240609");
        simpleobjects[2] = new SimpleObject2("s19", "kong", "240601");
        simpleobjects[3] = new SimpleObject2("s0", "keke", "240611");
        simpleobjects[4] = new SimpleObject2("s8", "choo", "240612");
        simpleobjects[5] = new SimpleObject2("s4", "jjonge", "240618");
        simpleobjects[6] = new SimpleObject2("s28", "janasddg", "240614");
        simpleobjects[7] = new SimpleObject2("s55", "gohhhh", "240605");
        simpleobjects[8] = new SimpleObject2("s33", "naeee", "240616");
        simpleobjects[9] = new SimpleObject2("s15", "yowwwu", "240617");
	}
}
