...
public class Test_실습3_7객체배열정렬검색_람다식 {
	static void swap(Fruit4[] data, int idx1, int idx2) {//스트링의 맞교환 함수로 sortData()에서 호출됨
		Fruit4 str = data[idx1]; data[idx1] = data[idx2]; data[idx2] = str;
...
//		System.out.println("\n comparator cc 객체를 사용:: ");
		Arrays.sort(arr, cc);
		showData("\nArrays.sort(arr, cc) Name 정렬 후", arr);
